...
class AlohaHD5Extractor:
    ...
    def extract_episode_frames(
        episode_path: str | Path, features: dict[str, dict], image_compressed: bool, task_description: str = ""
    ...
    def define_features(
        ...
        with h5py.File(hdf5_file_path, "r") as hdf5_file:
            ...
            for topic in topics:
                ...
                topic_shape = hdf5_file[topic][0].shape
                ...
                elif not topic_shape:
                    logging.warning(f"Skipping topic {topic} because it has no shape")
                    continue
...
class DatasetConverter:
    ...
    def extract_episode(self, episode_path, task_description: str = ""):
        ...
        # look for task_description in the episode_path.json file
        if not task_description: 
            abs_path = Path(episode_path).absolute().with_suffix('')
            comands_file = Path(str(abs_path) + '_text_commands.json')
            if comands_file.exists():
                with open(comands_file, "r") as f:
                    data = json.load(f)
                task_description = next(iter(data.values()))

        for frame in AlohaHD5Extractor.extract_episode_frames(
            episode_path, self.features, self.image_compressed, task_description
        ):
